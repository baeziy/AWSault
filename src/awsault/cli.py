...
examples:
  ...
  awsault --policy S3Access                  read a policy document live from AWS
  ...
  awsault --policy arn:aws:iam::123:policy/X read a managed policy by ARN
  awsault --all-policies                    list and read all policies on current identity
  awsault --all-policies --role BackendRole trust + all policies on a role
...
""",
    ...
    p.add_argument("--user", default=None, metavar="USERNAME", help="target a different IAM user (use with --policy or --all-policies)")
    p.add_argument("--role", default=None, metavar="ROLENAME", help="target a different IAM role (use with --policy or --all-policies)")
    return p
...
def main():
    ...
    # --user and --role are mutually exclusive
    if args.user and args.role:
        _die("--user and --role cannot be used together.")

    # --user/--role require --policy or --all-policies
    if (args.user or args.role) and not (args.policy or args.all_policies):
        _die("--user and --role require --policy or --all-policies.")

    # policy reader
    if args.policy:
        _cmd_policy(args.policy, args.version, args.profile, args.region, args.user, args.role)
        return

    # all policies
    if args.all_policies:
        _cmd_all_policies(args.profile, args.region, args.user, args.role)
        return
...
def _cmd_scan(args):
    ...
    if not multi_region:
        ...
        # --all-regions: permissions are checked once, resources are found per-region
        #
        # IAM permissions are region-independent. If an API call succeeds in
        # us-east-1, it will succeed in every other region too. What changes
        # across regions is which RESOURCES exist (instances, snapshots, etc).
        #
        # So the surface scan runs once to map permissions, and deep enumeration
        # runs per-region to discover resources.

        # --- Phase 1: Surface scan (once) ---
        con.print(f"[bold cyan]{'-' * 50}[/bold cyan]")
        con.print(f"[bold cyan]Permission Scan[/bold cyan]")
        con.print(f"  [dim]IAM permissions are region-independent. Scanning once to map access.[/dim]\n")
        ...
        if mode == "godeep":
            # --- Identity recon (once, global) ---
            con.print(f"\n[bold cyan]{'-' * 50}[/bold cyan]")
            con.print(f"[bold cyan]Identity Recon[/bold cyan]\n")
            con.print("[bold]Phase 2:[/bold] Deep enumeration (identity)")
            id_deep = _run_deep(session, quick, args.threads)
            all_deep.update(id_deep)

            recon = id_deep.get("iam_self")
            _print_recon(recon)

            # --- Per-region: deep enum, audit, loot ---
            con.print(f"\n[bold cyan]{'-' * 50}[/bold cyan]")
            con.print(f"[bold cyan]Resource Discovery ({len(regions)} regions)[/bold cyan]")
            con.print(f"  [dim]Permissions are the same everywhere. Now checking which regions have actual resources.[/dim]\n")

            for rgn in regions:
                con.print(f"  [bold cyan]--- {rgn} ---[/bold cyan]")

                rgn_session = creds.load_session(args.profile, rgn)

                con.print(f"  [bold]Deep enumeration[/bold]")
                deep_results = _run_deep(rgn_session, quick, args.threads)

                # check if anything was found in this region
                has_data = any(v for v in deep_results.values() if v)
                if not has_data:
                    con.print(f"    [dim]No resources found[/dim]")
                else:
                    for k, v in deep_results.items():
                        if v and k != "iam_self":
                            all_deep[f"{k} ({rgn})"] = v

                con.print(f"  [bold]Security audit[/bold]")
                findings = audit.run_audit(quick, deep_results)
                if findings:
                    con.print(f"    [bold]{len(findings)}[/bold] findings")
                    all_findings.extend(findings)
                else:
                    con.print(f"    [dim]No findings[/dim]")

                con.print(f"  [bold]Loot extraction[/bold]")
                loot_results = _run_loot(rgn_session, args.threads)
                has_loot = any(v for v in loot_results.values() if v)
                if has_loot:
                    ...
                    con.print(f"    [dim]No loot found[/dim]")

                con.print()

            # --- Combined findings across all regions ---
            if all_findings:
                con.print(f"[bold cyan]{'-' * 50}[/bold cyan]")
                con.print(f"[bold cyan]ALL FINDINGS ({len(all_findings)} across {len(regions)} regions)[/bold cyan]\n")
...
def _resolve_target(principal_type, principal_name, target_user, target_role):
    """Determine the target principal based on --user/--role flags."""
    if target_user:
...
def _raw_json(obj):
    """Dump an object as formatted JSON, handling datetimes."""
    ...
    return json.dumps(obj, indent=2, default=_ser)


def _cmd_policy(name, version, profile, region, target_user=None, target_role=None):
    """Read one or more policy documents live from AWS. Never auto-detects roles."""
    recon = _load_recon()
    iam, principal_type, principal_name = _init_policy_session(profile, region)
    tgt_type, tgt_name = _resolve_target(principal_type, principal_name, target_user, target_role)

    names = [n.strip() for n in name.split(",") if n.strip()]

    for n in names:
        # if it looks like an ARN, treat as managed policy ARN directly
        if n.startswith("arn:"):
            _read_managed_policy_by_arn(iam, n, version)
            continue

        found_as = _identify_policy(n, recon, iam, tgt_type, tgt_name)

        if found_as == "inline_policy":
            _read_inline_policy(iam, n, tgt_type, tgt_name)
        elif found_as == "managed_policy":
            _read_managed_policy(iam, n, version, recon)
        else:
            con.print(f"\n[red]'{n}' not found as a policy.[/red]")
            con.print(f"[dim]Searched: inline policies on {tgt_type}/{tgt_name}, "
                      f"managed policies (customer and AWS).[/dim]\n")


def _cmd_all_policies(profile, region, target_user=None, target_role=None):
    """List and read all policies attached to a user or role."""
    recon = _load_recon()
    iam, principal_type, principal_name = _init_policy_session(profile, region)
    tgt_type, tgt_name = _resolve_target(principal_type, principal_name, target_user, target_role)

    con.print(f"\n[bold cyan]{'-' * 50}[/bold cyan]")
    con.print(f"[bold cyan]ALL POLICIES FOR:[/bold cyan] [bold]{tgt_type}/{tgt_name}[/bold]\n")

    # if targeting a role, show trust policy first
    if tgt_type == "role":
        try:
            role_resp = iam.get_role(RoleName=tgt_name)
    ...
    try:
        if tgt_type == "user":
            resp = iam.list_user_policies(UserName=tgt_name)
        else:
            resp = iam.list_role_policies(RoleName=tgt_name)
        inline_names = resp.get("PolicyNames", [])
    except Exception as e:
        err = getattr(e, "response", {}).get("Error", {}).get("Code", str(e))
        action = "iam:ListUserPolicies" if tgt_type == "user" else "iam:ListRolePolicies"
        con.print(f"\n  [yellow]Inline policies:[/yellow] [red]Access denied[/red] "
    ...
    try:
        if tgt_type == "user":
            resp = iam.list_attached_user_policies(UserName=tgt_name)
        else:
            resp = iam.list_attached_role_policies(RoleName=tgt_name)
        managed = resp.get("AttachedPolicies", [])
    except Exception as e:
        err = getattr(e, "response", {}).get("Error", {}).get("Code", str(e))
        action = "iam:ListAttachedUserPolicies" if tgt_type == "user" else "iam:ListAttachedRolePolicies"
        con.print(f"\n  [yellow]Managed policies:[/yellow] [red]Access denied[/red] "
                  f"[dim]-- requires {action} ({err})[/dim]")

    total = len(inline_names) + len(managed)
    if total == 0:
        con.print(f"\n  [dim]No permission policies found (or insufficient permissions to list them).[/dim]\n")
        return

    con.print(f"\n  [bold]{total} permission policies found[/bold] "
              f"[dim]({len(inline_names)} inline, {len(managed)} managed)[/dim]\n")

    # display each inline policy
    for pname in inline_names:
        _read_inline_policy(iam, pname, tgt_type, tgt_name)
...
def _identify_policy(name, recon, iam, tgt_type, tgt_name):
    """Figure out what a name refers to: inline_policy or managed_policy. Never roles."""
    ...
    # 2. try live API calls
    # try inline policy on target user/role
    try:
        if tgt_type == "user":
            iam.get_user_policy(UserName=tgt_name, PolicyName=name)
            return "inline_policy"
        elif tgt_type == "role":
            iam.get_role_policy(RoleName=tgt_name, PolicyName=name)
            return "inline_policy"
...
def _read_inline_policy(iam, policy_name, tgt_type, tgt_name):
    ...
    try:
        if tgt_type == "user":
            resp = iam.get_user_policy(UserName=tgt_name, PolicyName=policy_name)
        elif tgt_type == "role":
            resp = iam.get_role_policy(RoleName=tgt_name, PolicyName=policy_name)
        else:
...
def _read_managed_policy_by_arn(iam, policy_arn, version):
    """Fetch and display a managed policy by its ARN directly (for cross-account etc.)."""
    con.print(f"\n[bold cyan]{'-' * 50}[/bold cyan]")
    con.print(f"[bold cyan]POLICY:[/bold cyan] [bold]{policy_arn}[/bold]\n")
    ...
    try:
        ...
        pname = pol_meta.get("PolicyName", "")

        con.print(f"  [dim]Name:[/dim]        {pname}")
    ...
    if len(all_versions) > 1 and not version:
        ...
        if other:
            con.print(f"\n  [dim]Tip: awsault --policy {policy_arn} --version {other[0]}[/dim]")
